...
COPY ./tna_account_management/static_src/ ./tna_account_management/static_src/
...
RUN useradd tna_account_management --create-home && mkdir /app $VIRTUAL_ENV && chown -R tna_account_management /app $VIRTUAL_ENV
...
USER tna_account_management

# Install your app's Python requirements.
RUN python -m venv $VIRTUAL_ENV
COPY --chown=tna_account_management pyproject.toml poetry.lock ./
RUN pip install --upgrade pip && poetry install ${POETRY_INSTALL_ARGS} --no-root --extras gunicorn

COPY --chown=tna_account_management --from=frontend ./tna_account_management/static_compiled ./tna_account_management/static_compiled

# Copy application code.
COPY --chown=tna_account_management . .

# Run poetry install again to install our project (so the the tna_account_management package is always importable)
...
# Load shortcuts
COPY ./docker/bashrc.sh /home/tna_account_management/.bashrc

# Run the WSGI server. It reads GUNICORN_CMD_ARGS, PORT and WEB_CONCURRENCY
# environment variable hence we don't specify a lot options below.
CMD gunicorn tna_account_management.wsgi:application
